...
type Service = { id: string; name: string; specialty: string; durationMinutes: number; price?: number | null };
type Visit = { id: string; status: string; startedAt: string; completedAt?: string | null; notes?: string | null };
type Appointment = { id: string; scheduledAt: string; durationMinutes: number; status: string; reason?: string | null; notes?: string | null; patient: Patient; doctor?: Doctor | null; service?: Service | null; visit?: Visit | null };
...
export default function AppointmentsPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([authenticatedFetch('/api/v1/appointments/doctors'), authenticatedFetch('/api/v1/organizations/me/services')]).then(async ([doctorsResponse, servicesResponse]) => {
      if (doctorsResponse.ok) setDoctors(await doctorsResponse.json() as Doctor[]);
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await authenticatedFetch('/api/v1/appointments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ patientId: patient.id, serviceId: serviceId || undefined, doctorId: doctorId || undefined, scheduledAt: new Date(`${scheduledDate}T${scheduledTime}:00`).toISOString(), durationMinutes: Number(duration), reason: reason || undefined }) });
      if (!response.ok) throw new Error(await getApiErrorMessage(response, 'تعذر حجز الموعد.'));
      setPatient(null); setSearch(''); setReason(''); setServiceId(''); setDoctorId(''); setScheduledDate(date); setScheduledTime(localTime()); await load(date);
  ...
  const testReminder = async (appointment: Appointment) => {
    if (!appointment.patient.phone) { setError('لا يوجد رقم هاتف محفوظ لهذا المريض.'); return; }
    if (!window.confirm(`سيتم إرسال تذكير حقيقي إلى ${appointment.patient.firstName} فقط. هل تؤكد؟`)) return;
    setWorkingId(appointment.id); setError('');
    try {
      const response = await authenticatedFetch(`/api/v1/whatsapp/appointments/${appointment.id}/test-reminder`, { method: 'POST' });
      if (!response.ok) throw new Error(await getApiErrorMessage(response, 'تعذر إرسال تذكير الاختبار.'));
      const result = await response.json() as { status: string; reason?: string };
      if (result.status === 'sent') await load(date);
      else setError(`لم يتم الإرسال: ${result.reason || 'سبب غير معروف'}`);
    } catch (reasonValue) { setError(reasonValue instanceof Error ? reasonValue.message : 'تعذر إرسال تذكير الاختبار.'); }
    finally { setWorkingId(''); }
  };
