...
export default function CampaignsPage() {
  ...
  const loadCampaigns = async () => {
    const [response, subscriptionResponse, patientsResponse] = await Promise.all([authenticatedFetch('/api/v1/whatsapp/campaigns'), authenticatedFetch('/api/v1/subscriptions/current'), authenticatedFetch('/api/v1/patients')]);
  ...
  const createCampaign = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true); setError(''); setNotice('');
    try {
      const response = await authenticatedFetch('/api/v1/whatsapp/campaigns', { method: 'POST', body: JSON.stringify({ ...filterPayload(filters), ...(selectedPatientIds.length ? { patientIds: selectedPatientIds } : {}), offerText: medicalOffer, ...(expiresAt ? { expiresAt: new Date(`${expiresAt}T12:00:00`).toISOString() } : {}) }) });
      ...
      setServiceName(''); setOfferText(''); setExpiresAt(''); setPreview(null); setSelectedPatientIds([]); await loadCampaigns();
