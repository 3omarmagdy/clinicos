...
export default function CampaignsPage() {
  ...
  const previewAudience = async () => {
    ...
    try {
      const response = await authenticatedFetch('/api/v1/whatsapp/campaigns/preview', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...filterPayload(filters), offerText: medicalOffer || 'عرض طبي تجريبي', ...(expiresAt ? { expiresAt: new Date(`${expiresAt}T12:00:00`).toISOString() } : {}) }) });
  ...
  const createCampaign = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await authenticatedFetch('/api/v1/whatsapp/campaigns', { method: 'POST', body: JSON.stringify({ ...filterPayload(filters), offerText: medicalOffer, ...(expiresAt ? { expiresAt: new Date(`${expiresAt}T12:00:00`).toISOString() } : {}) }) });
      ...
      setServiceName(''); setOfferText(''); setExpiresAt(''); setPreview(null); await loadCampaigns();
