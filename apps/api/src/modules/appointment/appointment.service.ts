...
export class AppointmentService {
  ...
  async updateStatus(id: string, organizationId: string, data: UpdateAppointmentStatusDto) {
    const appointment = await this.prisma.appointment.findFirst({ where: { id, organizationId }, select: { id: true } });
    if (!appointment) throw new NotFoundException('Appointment not found');
    return this.prisma.appointment.update({ where: { id }, data: { status: data.status }, include: { patient: { select: patientSelect } } });
  }
}
