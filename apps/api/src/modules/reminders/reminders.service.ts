...
export class RemindersService implements OnModuleInit, OnModuleDestroy {
  ...
  private async scheduleUpcoming() {
    ...
    const appointments = await this.prisma.appointment.findMany({
      where: { status: 'booked', scheduledAt: { gte: now, lte: new Date(target.getTime() + 10 * 60_000) }, patient: { email: { not: null } } },
  ...
  private async processDue() {
    ...
    for (const reminder of due) {
      if (reminder.appointment.status !== 'booked' || !reminder.patient.email) {
